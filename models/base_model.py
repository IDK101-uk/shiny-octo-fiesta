...
class BaseModel:
    ...
    def save(self):
        ...
        # Create timestamp for the model version
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_path = os.path.join(PATHS["model_dir"], f"{self.name}_{timestamp}")
        ...
        metadata_path = os.path.join(PATHS["model_dir"], f"{self.name}_{timestamp}_metadata.joblib")
